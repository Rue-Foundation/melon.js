...
const setupFund = async (portfolioName, from = setup.defaultAccount) => {
  ...
  const participation = addresses.Participation;
  const riskManagement = addresses.RMMakeOrders;
  const sphere = addresses.Sphere;
  ...
  const receipt = await gasBoost(versionContract.methods.setupFund, params, {
    from,
  });
